	...
	apiInterfaces.jsonHttpRequest('stocks.exchange', 443, '', function(error, response) {
		...
		if(response){
			for (var i=0; i < response.length - 1; i++){
				...
				if(response[i].market_name.toString() === config.symbol.toUpperCase()+"_BTC")
				{
					symbol_price = parseFloat(response[i].sell)*100000000;
				}
			}
		}
		callback(response.error ? response.error : error, symbol_price );
